...
export const HealthReportScreen = () => {
  const { navigate, user, cycleData, logs, todayWater, isPremium, language } = useApp();
  ...
  const handleDownload = async () => {
    if (!isPremium) {
      alert("Doctor View PDF download is a Premium feature. Upgrade to Premium to export!");
      navigate("premium");
      return;
    }
  ...
  const handleShare = async () => {
    if (!isPremium) {
      alert("Sharing Health Report is a Premium feature. Upgrade to Premium to share!");
      navigate("premium");
      return;
    }
  ...
  return (
    <MobileLayout gradient="linear-gradient(180deg, #FAF5FF 0%, #FDF2F8 50%, #FFF0F4 100%)">
      <div className="flex flex-col h-screen">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none pb-6 relative">
          <div ref={reportRef} className={!isPremium ? "blur-[6px] select-none pointer-events-none" : ""} style={{ background: "linear-gradient(180deg, #FAF5FF 0%, #FFF0F4 100%)" }}>
          ...
          {/* Paywall Overlay */}
          {!isPremium && (
            <div className="absolute inset-0 bg-[#FAF5FF]/30 backdrop-blur-[4px] flex flex-col items-center justify-start pt-16 px-6 z-40">
              <motion.div
                initial={{ scale: 0.9, y: 15 }}
                animate={{ scale: 1, y: 0 }}
                className="bg-white rounded-3xl p-6 shadow-2xl border border-gray-100 max-w-sm w-full text-center space-y-4"
              >
                <div className="w-14 h-14 rounded-full bg-[#FFF0F2] flex items-center justify-center text-2xl mx-auto shadow-sm">
                  👑
                </div>
                <h3 className="text-[17px] font-bold text-gray-900" style={{ fontFamily: "Instrument Sans, sans-serif" }}>
                  Unlock Monthly Health Report
                </h3>
                <p className="text-gray-500 text-xs leading-relaxed">
                  Analyze cycle trends, symptom correlations, wellness logs, and get a shareable PDF report to review with your doctor.
                </p>
                
                <div className="bg-purple-50 rounded-2xl p-3 text-left space-y-2 text-purple-700 text-xs font-semibold">
                  <div className="flex items-center gap-2">
                    <span>✨</span> Advanced predictions accuracy
                  </div>
                  <div className="flex items-center gap-2">
                    <span>📊</span> Detailed symptom severity charts
                  </div>
                  <div className="flex items-center gap-2">
                    <span>🩺</span> Doctor-ready PDF export download
                  </div>
                </div>
                ...
                <div className="pt-2 space-y-2">
                  ...
                  <button
                    onClick={() => navigate("insights")}
                    className="w-full py-2.5 text-gray-400 font-semibold text-xs"
                  >
                    Go Back to Insights
                  </button>
                </div>
